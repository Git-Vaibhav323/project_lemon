...
export default function PlantCard({ plant, index, onClick }) {
  ...
  return (
    <article
      ...
      className="shop-card-entry card-tilt group cursor-pointer rounded-3xl bg-white border border-brand-bark/10 overflow-hidden flex flex-col"
    ...
    >
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle at var(--sx, 50%) var(--sy, 50%), rgba(74,122,69,0.1) 0%, transparent 60%)",
        }}
      />
      ...
      >
        ...
        {plant.badge && (
          <span 
            className="absolute top-2 left-2 sm:top-3 sm:left-3 bg-brand-green text-brand-dark text-[10px] sm:text-xs font-semibold px-2.5 py-1 rounded-full shadow-lg z-10"
      ...
      >
        ...
        <div className="flex items-center justify-between mt-3 sm:mt-4 pt-3 border-t border-brand-bark/10">
          <span className="text-brand-dark font-bold text-sm sm:text-base">Rs. {plant.price}/-</span>
          <button
            type="button"
            onClick={handleCartClick}
            className={`cart-btn w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-xl border border-brand-bark/18 text-brand-dark/75 hover:bg-brand-sand hover:border-white/25 transition-all duration-300 ${
              cartPulse ? "scale-125" : ""
            }`}
