      ...
      <div className="relative z-10 flex flex-col h-full p-6 sm:p-7">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div
            ...
            style={{ background: review.avatarColor || "#4a7a45", color: "#000" }}
            aria-hidden="true"
          >
            {review.initials}
          </div>
          <div className="min-w-0">
            <p className="text-brand-dark text-sm sm:text-base font-semibold">{review.name}</p>
            <StarRating count={review.rating || 5} size={12} />
          </div>
        </div>

        {/* Review text */}
        <p className="text-brand-dark/55 text-xs sm:text-sm leading-relaxed flex-1">
          {review.text}
        </p>
      </div>
